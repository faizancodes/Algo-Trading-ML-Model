...
def classifyData():
    ...
    with open('C:\\Users\\faiza\\OneDrive\\Desktop\\' + stock + 'PriceLabels2012.csv') as csvfile:
...
def convertDataset(symbol):
    ...
    with open('C:\\Users\\faiza\\OneDrive\\Desktop\\' + symbol + 'PriceLabels2012.csv') as csvfile:
    ...
    for row in data:
        ...
        mlData.append([row[0 : 7], pcts, inds, rating])


    MyFile = open('C:\\Users\\faiza\\OneDrive\\Desktop\\' + symbol + 'MLDataset2012.csv','w')
...
def trainModel(symbol):
    ...
    print('Pos Error:', str(posError / posCounter))
    print('Neg Error:', str(negError / negCounter))

'''

    convertDataset(symbol)

    newFeatures = pd.read_csv('C:\\Users\\faiza\\OneDrive\\Desktop\\AAPLMLDataset.csv')
    
    newFeatures = pd.get_dummies(newFeatures)
    print('\n\n\nNew Features:', newFeatures)

    newLabels = np.array(newFeatures['Buy'])

    newFeatures = newFeatures.drop('Buy', axis = 1)
    newFeatures = newFeatures.drop('Hold', axis = 1)
    newFeatures = newFeatures.drop('Sell', axis = 1)
    #newFeatures = newFeatures.drop('Date', axis = 1)

    feature_list2 = list(newFeatures.columns)

    newFeatures = np.array(newFeatures)

    train_features2, test_features2, train_labels2, test_labels2 = train_test_split(newFeatures, newLabels, test_size = 0.25, random_state = 42)
