...
def classifyData():
    ...
    with open('C:\\Users\\faiza\\OneDrive\\Desktop\\' + stock + 'PriceLabels.csv') as csvfile:
        ...
        for row in readCSV:
            ...
            if convertable(counter, 20) == True and adjClose > getMovingAverage(counter, 20):
            ...
            elif convertable(counter, 20) == True and adjClose < getMovingAverage(counter, 15):
...
def plotGraph():
    ...
    for x in range(len(classifiedData)):
        closes.append(classifiedData[x][0][6])

    closes.pop(0)
    print(len(dates), len(closes))
