...
def classifyData():
    ...
    with open('C:\\Users\\faiza\\OneDrive\\Desktop\\' + stock + 'PriceLabels.csv') as csvfile:
        ...
        for row in readCSV:
            ...
            if convertable(counter, 20) == True and (adjClose > getMovingAverage(counter, 20) and rsiVal < 70):
            ...
            if convertable(counter, 20) == True and (adjClose < getMovingAverage(counter, 12) or rsiVal >= 70):
...
def checkPerformance(symbol):
    ...
    with open('C:\\Users\\faiza\\OneDrive\\Desktop\\' + symbol + 'PriceLabelsPerformance.csv', 'w') as f:
        ...
        f.write('\nRatio:,' + str(winningTrades / losingTrades))
        f.write('\nAvg Win%:,' + str(avgWin / winningTrades) + '%')
        f.write('\nAvg Loss%:,' + str(avgLoss / losingTrades) + '%')
